...
case class Exists(id:String, url:String, selfLink:String)

case class Post(title:String, content:String, added:Option[Exists]=None) {
  def json(blogId:String):String = {
    ...
    if (!added.isEmpty) {
      val Exists(id, url, selfLink) = added.get
